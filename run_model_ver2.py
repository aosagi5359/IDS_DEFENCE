...
def block_port_local(port, protocol='TCP'):
    """使用 PowerShell New-NetFirewallRule 封鎖指定端口（修復編碼問題）"""
    with lock:
        try:
            rule_name = f"IDS_Block_Port_{port}_{protocol}"
            ps_cmd = f'New-NetFirewallRule -DisplayName "{rule_name}" -Direction Inbound -Action Block -LocalPort {port} -Protocol {protocol}'
            # 關鍵：改用 cp950 解碼（繁體中文 Windows 標準）
            result = subprocess.run(
                ["powershell", "-Command", ps_cmd],
                capture_output=True,
                text=True,
                encoding='cp950',        # ← 改這裡
                errors='replace'         # ← 即使有無法解碼的字元也用 � 替代，不會崩潰
            )
            if result.returncode != 0:
                logger.error(f"無法封鎖端口 {port} ({protocol})：{result.stderr.strip()}")
                return False
...
def unblock_port_local(port, protocol='TCP'):
    """使用 PowerShell Remove-NetFirewallRule 解除封鎖指定端口（修復編碼問題）"""
    with lock:
        try:
            rule_name = f"IDS_Block_Port_{port}_{protocol}"
            ...
            # PowerShell 如果規則不存在會報錯，但我們加了 -ErrorAction SilentlyContinue，returncode 仍為 0
...
class IDSApp:
    ...
    def refresh_blocked_ports(self):
        ...
        try:
            # 使用管道正確取得端口與協議
            ps_cmd = '''
            Get-NetFirewallRule -DisplayName "IDS_Block_Port_*" | 
            ForEach-Object {
                $rule = $_
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for line in lines:
                    ...
                    if len(parts) >= 2:
                        try:
                            port = parts[0].strip()
                            protocol = parts[1].strip().upper()
                            if port.isdigit() or port == 'Any':  # 處理可能的 Any
                                self.ports_tree.insert("", tk.END, values=(port, protocol))
                        except:
                            continue  # 跳過非數據行
                if not self.ports_tree.get_children():
                    self.log_message("目前沒有 IDS 封鎖的端口規則（或解析失敗）")
            else:
                self.log_message("目前沒有 IDS 封鎖的端口規則")
                if result.stderr:
                    self.log_message(f"PowerShell 錯誤：{result.stderr.strip()}")
        except Exception as e:
